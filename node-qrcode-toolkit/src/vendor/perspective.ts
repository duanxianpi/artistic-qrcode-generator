...
import { createCanvas, Canvas, CanvasRenderingContext2D, ImageData, DOMMatrix} from "canvas";
...
export default class Perspective {
  ...
  constructor(ctxd: CanvasRenderingContext2D, target: ImageData) {
    ...
    let cvso = createCanvas(Math.round(target.width),Math.round(target.height));
    ...
    let cvst = createCanvas(ctxd.canvas.width,ctxd.canvas.height);
  ...
  draw(q: Quadrilateral) {
    ...
    if (base_index % 2 == 0) {
      ...
      for (let y = 0; y < oh; y += step) {
        ...
        ctxl.setTransform(new DOMMatrix([1, 0, 0, 1, 0, -y]));
        ...
        //
        ctxt.setTransform(new DOMMatrix([1, 0, 0, 1, 0, 0]));
      }
      ...
      for (let x = 0; x < ow; x += step) {
        ...
        ctxl.setTransform(new DOMMatrix([1, 0, 0, 1, -x, 0]));
        ctxl.drawImage(ctxo.canvas, 0, 0);
        //
        ctxt.translate(sx, sy);
        ctxt.rotate(ag);
